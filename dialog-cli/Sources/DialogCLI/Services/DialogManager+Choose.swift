...
extension DialogManager {
    ...
    func choose(_ request: ChooseRequest) -> ChoiceResponse {
        ...
        let swiftUIDialog = SwiftUIChooseDialog(
            ...
            position: position,
            onComplete: { selectedIndices, otherText, feedback in
                if selectedIndices.isEmpty && otherText == nil {
                    result = self.makeChoiceResponse(cancelled: true, feedbackText: feedback)
                    ...
                    result = self.makeChoiceResponse(answer: .multiple(selected), descriptions: descs, feedbackText: feedback)
                } else if let other = otherText, !other.isEmpty {
                    result = self.makeChoiceResponse(answer: .single(other), feedbackText: feedback)
                } else if let idx = selectedIndices.first {
                    result = self.makeChoiceResponse(answer: .single(request.choices[idx]), description: request.descriptions?[safe: idx], feedbackText: feedback)
                }
                NSApp.stopModal()
            },
            onCancel: { feedback in
                result = self.makeChoiceResponse(cancelled: feedback == nil, feedbackText: feedback)
                NSApp.stopModal()
            },
            onSnooze: { minutes in
